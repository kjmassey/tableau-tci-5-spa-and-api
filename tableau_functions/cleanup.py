...
def filter_items_on_tableau_server(req_data) -> list:
    """
    Filter a list of workbooks on Tableau Server by their last updated date. Return a list of results
    """

    # Lines 7-15 allow us to shorthand this every time
    ...
    with server.auth.sign_in(tab_auth):
        ...
        filtered_wbs = list(
            filter(
                lambda x: x.updated_at.timestamp()
                < datetime.strptime(
                    req_data["selectedDate"].split("T")[0], "%Y-%m-%d"
                ).timestamp(),
                all_wbs,
            )
        )
